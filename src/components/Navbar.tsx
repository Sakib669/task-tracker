  ...
  const handleSubscribe = async () => {
    ...
    try {
      const response = await fetch("/api/create-checkout", {
        ...
        // no body needed; the server creates a checkout for the current user
      });
    ...
    >
      <motion.div variants={itemVariants} className="text-center">
        ...
        <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
          Upgrade to premium and unlock powerful features to boost your
          productivity.
        </p>
      </motion.div>
          ...
          <Card className="relative h-full border-0 shadow-lg">
            ...
            <CardFooter>
              <Button variant="outline" className="w-full h-11" disabled>
                {isPremium ? "Downgrade first" : "Current Plan"}
              </Button>
            </CardFooter>
          </Card>
        </motion.div>
